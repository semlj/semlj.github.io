...
In this example, we demonstrate how to use a variances-covariances matrix as input data of `r modulename()`. We show input of `r modulename()` _syntax sub-module_ and  _interactive sub-module_, with particular focus on the latter one. We focus more on the setup of the data, so the example would not be interpreted as for other cases.
...
We start with the case in which variances and covariances are available. The data containing the variances and covariances of the observed variables are in the file `r datafile("HS_covs_whole.csv","HS_covs_whole.csv")`.  
