...
In `r modulename` we set the variables roles in the first panel as shown here: The dependent variable goes in the `r opt("Latent Endogenous Variables")` field, and the independent variable goes in the `r opt("Latent Exogenous Variables")` field.
...
We can see that the two coefficients we want to equate are labelled $p4$ and $p5$. Thus, we go to the `r opt ("Custom Model Settings")` panel and constraints them as equal.
...
We obtain this model by adding `l_arith` to the `r opt("Latent Endogeneous Variables")` field, filling it with the observed variable `arith`, and then defining `l_arith` predictors in the `r opt("Endogenous Models")` panel.
